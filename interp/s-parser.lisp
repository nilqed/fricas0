...
(DEFUN |init_parser_properties| #1=()
  (PROG #1#
    (RETURN
     (PROGN
      ((LAMBDA (|bfVar#1| |j|)
...
(DEFUN |pop_stack_1| #1=() (PROG #1# (RETURN (POP |$reduction_stack|))))
...
(DEFUN |top_of_stack| #1=() (PROG #1# (RETURN (CAR |$reduction_stack|))))
...
(DEFUN |parse_SPADSTRING| #1=() (PROG #1# (RETURN (|parse_token| 'SPADSTRING))))
 
; parse_KEYWORD() == parse_token("KEYWORD")
 
(DEFUN |parse_KEYWORD| #1=() (PROG #1# (RETURN (|parse_token| 'KEYWORD))))
 
; parse_ARGUMENT_DESIGNATOR() == parse_token("ARGUMENT-DESIGNATOR")
 
(DEFUN |parse_ARGUMENT_DESIGNATOR| #1=()
  (PROG #1# (RETURN (|parse_token| 'ARGUMENT-DESIGNATOR))))
 
; parse_SPADFLOAT() == parse_token("SPADFLOAT")
 
(DEFUN |parse_SPADFLOAT| #1=() (PROG #1# (RETURN (|parse_token| 'SPADFLOAT))))
 
; parse_IDENTIFIER() == parse_token("IDENTIFIER")
 
(DEFUN |parse_IDENTIFIER| #1=() (PROG #1# (RETURN (|parse_token| 'IDENTIFIER))))
 
; parse_NUMBER() == parse_token("NUMBER")
 
(DEFUN |parse_NUMBER| #1=() (PROG #1# (RETURN (|parse_token| 'NUMBER))))
...
(DEFUN |parse_new_expr| #1=()
  (PROG #1# (RETURN (PROGN (SETQ |$reduction_stack| NIL) (|parse_Expr| 0)))))
 
; parse_InfixWith() ==
;     not(parse_With()) => nil
;     push_form2("Join", pop_stack_2(), pop_stack_1())
 
(DEFUN |parse_InfixWith| #1=()
  (PROG #1#
    (RETURN
...
(DEFUN |parse_Expr1000| #1=() (PROG #1# (RETURN (|parse_Expr| 1000))))
     ...
     (COND ((NULL (|parse_Primary|)) NIL)
           (#2='T
            (PROGN
             ((LAMBDA #1#
                (LOOP
                 (COND ((NOT (|parse_Selector|)) (RETURN NIL)) (#2# NIL)))))
             (COND
              ((|parse_Application|)
...
(DEFUN |parse_PrimaryNoFloat| #1=()
  (PROG #1# (RETURN (AND (|parse_Primary1|) (OPTIONAL (|parse_TokTail|))))))
 
; parse_Primary() == OR(parse_Float(), parse_PrimaryNoFloat())
 
(DEFUN |parse_Primary| #1=()
  (PROG #1# (RETURN (OR (|parse_Float|) (|parse_PrimaryNoFloat|)))))
...
(DEFUN |parse_Float| #1=() (PROG #1# (RETURN (|parse_SPADFLOAT|))))
...
(DEFUN |parse_IntegerTok| #1=() (PROG #1# (RETURN (|parse_NUMBER|))))
 
; parse_FormalParameter() == parse_ARGUMENT_DESIGNATOR()
 
(DEFUN |parse_FormalParameter| #1=()
  (PROG #1# (RETURN (|parse_ARGUMENT_DESIGNATOR|))))
 
; parse_String() == parse_SPADSTRING()
 
(DEFUN |parse_String| #1=() (PROG #1# (RETURN (|parse_SPADSTRING|))))
 
; parse_VarForm() == parse_IDENTIFIER()
 
(DEFUN |parse_VarForm| #1=() (PROG #1# (RETURN (|parse_IDENTIFIER|))))
 
; parse_Name() == parse_IDENTIFIER()
 
(DEFUN |parse_Name| #1=() (PROG #1# (RETURN (|parse_IDENTIFIER|))))
...
(DEFUN |parse_IteratorTail| #1=()
  (PROG #1#
    (RETURN
     (COND
      ((|match_symbol| '|repeat|)
       (COND ((|repetition| NIL #'|parse_Iterator|) T)
             (#2='T (|push_reduction| '|null_Tail| NIL))))
      (#2# (|repetition| NIL #'|parse_Iterator|))))))
