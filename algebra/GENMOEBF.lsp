...
(SDEFUN |GENMOEBF;generalizedMoebiusFunction;LM$;4|
        ...
        (SPROG
         ((#1=#:G396 NIL) (|mf| (|Union| (|Matrix| R) "failed"))
          (|zf| (|Matrix| R)) (#2=#:G400 NIL) (|x| NIL) (#3=#:G399 NIL)
          (#4=#:G398 NIL) (|y| NIL) (#5=#:G397 NIL) (|xxo| (|List| P)))
         (SEQ
          (EXIT
           (SEQ
            (LETT |xxo|
                 ...
                 (SEQ
                  (COND
                   ((SPADCALL |pi| |si| (QREFELT $ 12))
                    (PROGN (LETT #1# (|spadConstant| $ 25)) (GO #2=#:G402))))
                  (EXIT (|spadConstant| $ 17))))
                #2# (EXIT #1#)))) 

(SDEFUN |GENMOEBF;apply;$2PR;6| ((|mf| $) (|x| P) (|y| P) ($ R))
        (SPROG
         ((#1=#:G409 NIL) (|ky| #2=(|Integer|)) (|kx| #2#)
          (|mfn| (|Matrix| R)))
         (SEQ
          (EXIT
...
(DEFUN |GeneralizedFiniteMoebiusFunction| (&REST #1=#:G415)
  (SPROG NIL
         (PROG (#2=#:G416)
           (RETURN
            (COND
