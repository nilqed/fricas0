...
(SDEFUN |IPADIC;=;2$B;15| ((|x| $) (|y| $) ($ |Boolean|))
        (SPROG
         ((|st| (|Stream| (|Integer|))) (#1=#:G455 NIL) (#2=#:G456 NIL)
          (|i| NIL) (|n| (|Integer|)))
         (SEQ
          (EXIT
           (SEQ
...
(SDEFUN |IPADIC;iRoot!0| (($$ NIL))
        (PROG (|f| |alpha| $ |pPow| |invFpx0| |p|)
          ...
          (RETURN
           (PROGN
            (SPROG ((#1=#:G604 NIL) (|num| NIL) (|digit| NIL))
                   (SEQ
                    (LETT |num|
                          (SPADCALL
                           (PROG2
...
(DEFUN |InnerPAdicInteger| (&REST #1=#:G651)
  (SPROG NIL
         (PROG (#2=#:G652)
           (RETURN
            (COND
