...
(SDEFUN |ODETOOLS;wronskianMatrix;LNniM;2|
        ...
        (SPROG
         ((|v| (|Vector| F)) (#1=#:G712 NIL) (|i| NIL) (|m| (|Matrix| F)))
         (SEQ
...
(SDEFUN |ODETOOLS;wronskianMatrix;LNniM;2!0| ((|f1| NIL) ($$ NIL))
        (PROG (|diff| $)
          (LETT |diff| (QREFELT $$ 1)
                . #1=(|ODETOOLS;wronskianMatrix;LNniM;2|))
          (LETT $ (QREFELT $$ 0) . #1#)
...
(SDEFUN |ODETOOLS;particularSolution;LODOFLMU;4|
        ((|op| LODO) (|g| F) (|b| |List| F) (|integration| |Mapping| F F)
         ($ |Union| F "failed"))
        (SPROG
         ((|ans| (F)) (#1=#:G733 NIL) (|f| NIL) (|i| NIL) (|s| (|Vector| F))
         ...
         (PROG (#2=#:G735)
           (RETURN
            (COND
             ...
             ('T
              (UNWIND-PROTECT
                  (PROG1 (APPLY (|function| |ODETools;|) #1#)
                    (LETT #2# T . #3#))
